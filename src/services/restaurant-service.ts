...
const parseRestaurantFeature = (feature: any): Restaurant | null => {
  ...
  return {
    ...
    latitude: props.latitude ?? (coordinates ? coordinates[1] : undefined),
    longitude: props.longitude ?? (coordinates ? coordinates[0] : undefined),
...
const parseGeoJSONRestaurants = (data: any): Restaurant[] => {
  ...
  if (data && data.type === "FeatureCollection" && Array.isArray(data.features)) {
    return data.features
      .map((feature: any) => parseRestaurantFeature(feature))
...
export const restaurantService = {
  ...
  async getRestaurant(id: string): Promise<Restaurant> {
    const response = await api.get<any>(ENDPOINTS.RESTAURANTS_DETAIL(id))
    const data = response.data

    if (data && data.type === "Feature") {
      return parseRestaurantFeature(data) || data
    }
    if (data && data.properties && (data.id || data.properties.id)) {
      return parseRestaurantFeature(data) || data
    }
    return data
  },
